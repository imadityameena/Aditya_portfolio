...
const skills = [
  { icon: Code2, label: 'Languages', items: ['C++', 'JavaScript', 'TypeScript', 'Python'] },
  { icon: Sparkles, label: 'Frontend', items: ['React.js', 'Next.js', 'Tailwind CSS'] },
  { icon: Server, label: 'Backend', items: ['Node.js', 'Express.js', 'REST APIs', 'MVC'] },
  { icon: Database, label: 'Data + AI', items: ['MongoDB', 'PostgreSQL', 'OpenAI API', 'Gemini AI'] },
]
...
const projects = [
  {
    ...
    impact: '3-role healthcare platform with conflict-free bookings and dual payment gateways.',
    points: ['Patient, doctor, and admin dashboards', 'Slot validation for reliable scheduling', 'Stripe and Razorpay payments'],
  },
  {
    name: 'ChefAI',
    year: '2026',
    ...
    impact: 'Context-aware recipe generation from real pantry inventory and dietary preferences.',
    points: ['Inventory-aware meal plans', 'MVC REST API with secure auth', 'Automated shopping list generation'],
  },
]
    ...
    <main>
      ...
      <section id="top" className="hero">
        <motion.div className="hero-copy" style={{ y: heroY }}>
          ...
          <motion.p className="hero-summary" initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9, delay: 0.18 }}>
            {profile.summary}
          </motion.p>
          <motion.div className="hero-actions" initial={{ opacity: 0, y: 22 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9, delay: 0.28 }}>
            <MagneticLink href="#projects" className="primary-cta">
              View work <ArrowUpRight size={18} />
            </MagneticLink>
            {contactLinks.map(({ href, label, icon: Icon }) => (
              <MagneticLink key={label} href={href} className="icon-link" aria-label={label}>
                <Icon size={18} />
              </MagneticLink>
            ))}
          </motion.div>
        ...
        <motion.div className="hero-panel" initial={{ opacity: 0, scale: 0.94, rotateX: 8 }} animate={{ opacity: 1, scale: 1, rotateX: 0 }} transition={{ duration: 1, delay: 0.18, ease: [0.22, 1, 0.36, 1] }}>
      ...
      <section className="ticker" aria-label="Technical focus">
        <motion.div animate={{ x: ['0%', '-50%'] }} transition={{ duration: 22, repeat: Infinity, ease: 'linear' }}>
          {['React', 'Node.js', 'PostgreSQL', 'MongoDB', 'REST APIs', 'Gemini AI', 'OpenAI API', 'Competitive Programming', 'System Design'].concat(['React', 'Node.js', 'PostgreSQL', 'MongoDB', 'REST APIs', 'Gemini AI', 'OpenAI API', 'Competitive Programming', 'System Design']).map((item, index) => (
      ...
      <section id="work" className="section two-column">
        <Reveal>
          ...
          <h2>Built production systems, fixed real outages, and shipped fast.</h2>
        </Reveal>
        <Reveal delay={0.1} className="timeline-card">
          <div className="timeline-head">
            ...
            <span><CalendarDays size={15} /> May 2025 - Nov 2025</span>
          </div>
          <div className="timeline-list">
            {work.map((item, index) => (
              <motion.div key={item} className="timeline-item" initial={{ opacity: 0, x: 18 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.07 }}>
      ...
      <section id="projects" className="section">
        ...
        <div className="project-grid">
          {projects.map((project, index) => (
            <Reveal key={project.name} delay={index * 0.12} className="project-card">
